...
class SortableGrid extends Component {
  ...
  componentWillReceiveProps = (properties) => this.handleNewProps(properties, true)

  handleNewProps = (properties, is_received_props) => {
    let is_updated = false;
    if (_.size(properties.children) !== (this.old_properties ? _.size(this.old_properties.children) : 0)) {
      this.itemOrder = [];
      this.items = [];
      is_updated = true;
    }
    ...
    this._saveItemOrder(properties.children, is_updated)
  ...
  blockAnimateFadeOut = () => {
    ...
    return new Promise((resolve, reject) => {
      Animated.timing(
        this.state.deleteBlockOpacity, { toValue: 0, duration: 2 * this.activeBlockCenteringDuration }
      ).start(resolve)
    })
  }
  ...
  _saveItemOrder = (items, is_updated) => {
    items.forEach((item, index) => {
      if (is_updated) {
        this.itemOrder.push({ key: item.key, ref: item.ref, order: index });
  ...
  _getImageDeleteIconStyle = (key) => [{
    ...
    this.state.activeBlock == key &&
    this._getBlock(key).origin && { opacity: .5 + this._getDynamicOpacity(key) }
  ]
